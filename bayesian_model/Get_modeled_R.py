...
with open("fit_data_pyro_base.json","r") as f:
    params = json.load(f)
...
n_days = 153
n_years = 10
Fips = np.array(list(itertools.chain(*[itertools.repeat(i, n_days*10) for i in crosswalk.keys()])))
...
lam = torch.exp((torch.tensor(betas_means) * X_).sum(-1).clamp(-20, 10))  # baseline rate
tau = torch.sigmoid((torch.tensor(gammas_means) * X_).sum(-1))  # heat alert effectiveness
...
df.to_csv("../Summer_results/Bayesian_R_7-12.csv")