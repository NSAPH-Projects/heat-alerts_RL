...
# cfg.model_name="Tune_F-Q_D10_Rstr-HI-0.5_arch-2-32_ns-2048_fips-41067"
# cfg.county=41067
# cfg.forecasts.forecast_type="[Q, D10]" 
# cfg.restrict_days.HI_restriction=0.5
# cfg.algo.policy_kwargs.net_arch=[32,32]
...
def main(cfg: DictConfig):
    ...
    if diff_timesteps < 5000000: # best model might still be coming?
        ...
        # Read in the partially-trained model:
        rl_model.load(f"./logs/SB/{cfg.model_name}/best_model/best_model")
        rl_model.set_env(env) # not sure if this is necessary 
        ...
        rl_model.learn(
            total_timesteps=cfg.training_timesteps - best_timestep, # difference from where it saved
            callback=[eval_callback, alert_logging_callback],
            progress_bar=True,
        )

    ## Do evaluations regardless:
