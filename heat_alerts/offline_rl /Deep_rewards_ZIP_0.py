...
## Trying idea to account for zero-inflation by first fitting a classifier for whether it's a zero
...
def main(params):
    ...
    train_tensors[0] = torch.FloatTensor(train_tensors[0])
    train_tensors[1] = torch.LongTensor(train_tensors[1])
    val_tensors[0] = torch.FloatTensor(val_tensors[0])
    ...
    model = Logit_Lightning(state_dim, config, N = N, **params)
