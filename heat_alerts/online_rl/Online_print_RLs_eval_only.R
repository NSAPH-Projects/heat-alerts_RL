    ...
    for(nhl in NHL){
      for(nhu in NHU){
        for(s in n_steps){
          ...
          cat(paste0("python old_evaluation_SB3.py policy_type=RL eval.val_years=false eval.match_similar=true ", "county=", county, " r_model=", r_model,
              ...
              paste0("python old_evaluation_SB3.py policy_type=RL eval.val_years=true eval.match_similar=false ", "county=", county, " r_model=", r_model,
                     " restrict_days=none", " forecasts=none", " algo.policy_kwargs.net_arch=", arch, " algo.n_steps=", s,
                     " model_name=", r_model, "_", algo, "_F-none", "_fips-", county, "\n")
          )
